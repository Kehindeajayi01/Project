...
data_path = "C:/Users/ajayi/OneDrive/Desktop/My_research/PubChem_data.csv"
pubchem = pd.read_csv(data_path)

mat_name = pubchem.loc[:, "cmpdname"]
...
"""
Function to look up a word in a PubChem database and check if it is a material or not
"""
def get_material(word, materialLookup, material_embed):
    mat = "other"
    for elem in materialLookup:
        if elem.lower() == word:
            mat = "material"
        elif elem == word: 
            mat = "material"
    return material_embed[mat] 
...
def createMatrices(sentences, word2Idx, label2Idx, case2Idx, char2Idx, mat2Idx):
    ...
    for sentence in sentences:
        ...
        for word, char, label in sentence:
            ...
            matIndices.append(get_material(word, mat_name, mat2Idx))
            charIndices.append(charIdx)
            labelIndices.append(label2Idx[label])

        dataset.append([wordIndices, caseIndices, charIndices, labelIndices, matIndices])
...
def iterate_minibatches(dataset, batch_len):
    ...
    for i in batch_len:
        ...
        yield np.asarray(labels), np.asarray(tokens), np.asarray(caseing), np.asarray(char), np.asarray(materials)        
